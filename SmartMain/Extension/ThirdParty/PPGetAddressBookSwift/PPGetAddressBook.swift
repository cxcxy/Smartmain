...
/// 获取按A~Z顺序排列的所有联系人的闭包
public typealias AddressBookDictClosure = (_ addressBookDict: [String:[PPPersonModel]], _ nameKeys: [String], _ phoneArr: [String])->()
...
//        PPAddressBookHandle.req
        PPAddressBookHandle().requestAuthorizationWithSuccessClosure(success: {
            print("授权成功")
//            return true
        }) {
            print("授权失败")
...
//        return PPAddressBookHandle().requestAuthorizationWithSuccessClosure()
        ...
        queue.async {
            ...
            PPAddressBookHandle().getAddressBookDataSource(personModel: { (model) in
                ...
                if model.mobileArray.count > 0 {
                     phoneArr.append(model.mobileArray[0])
                }
            ...
            // 将排序好的通讯录数据回调到主线程
            DispatchQueue.main.async {
                success(addressBookDict, nameKeys,phoneArr)
            }
