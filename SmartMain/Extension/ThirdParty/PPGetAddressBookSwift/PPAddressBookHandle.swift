...
class PPAddressBookHandle {
    ...
    func requestAuthorizationWithSuccessClosure(success: @escaping ()->(),failure: @escaping () -> ()) {
    ...
    private func getDataSourceFrom_IOS9_Later(personModel success: PPPersonModelClosure, authorizationFailure failure: AuthorizationFailure) {
        ...
        for contact in contacts {
            ...
            for labelValue in contact.phoneNumbers {
                ...
                let phone_str_arr = phoneNumber.components(separatedBy: whitespace)
...
//                print(phone_Str)
                if phone_Str.validateMobile() {
                    model.mobileArray.append(phone_Str)
                }
