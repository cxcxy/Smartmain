...
class CyclePictureCell: UICollectionViewCell {
    ...
    var imageSource: ImageSource = ImageSource.local(name: ""){
        didSet {
            ...
            case let .network(urlStr):
               ...
               self.imageView.set_Img_Url(urlStr)
    ...
    var detailLableTextFont: UIFont = UIFont.systemFont(ofSize: 16) {
        didSet {
    ...
    var detailLableBackgroundColor: UIColor = UIColor.init(hexString: "45312f")!.withAlphaComponent(0.5) {
        didSet {
            detailLable.backgroundColor = detailLableBackgroundColor
        }
    }
    
    var detailLableHeight: CGFloat = 40 {
        didSet {
...
//        self.backgroundColor = UIColor.grayColor()
    ...
    fileprivate func setupDetailLable() {
        ...
        detailLable.backgroundColor = detailLableBackgroundColor
//        detailLable.textAlignment   =
         detailLable.font = detailLableTextFont
        detailLable.isHidden        = true //默认是没有描述的，所以隐藏它
//        detailLable.backgroundColor = UIColor.red
    ...
    override func layoutSubviews() {
        ...
        if let _ = self.imageDetail {
            ...
            let lableY: CGFloat = self.bounds.height - lableH
